defmodule DoitMcp.ImportGateTest do
  ...
  describe "shape_facts_block/3 (m03.04 3.1 iteration 1)" do
    ...
    test "a mirror batch gets both tells and the naming line" do
      ops = [
        add("docs/PLAN.md", String.duplicate("x", 2_000)),
        add("docs\\BACKLOG.md", String.duplicate("x", 2_500)),
        add("Real work item")
      ]

      block = ImportGate.shape_facts_block(ops, 3, 3)

      assert block =~ "Server-computed: this batch adds 3 tasks (3 this session)."
      assert block =~ "2 of 3 new task titles look like file paths/names."
      assert block =~ "2 new descriptions run 2000+ characters (whole-file sized)."
      assert block =~ "file-mirror tells"
    end

    test "a clean batch is just the size line" do
      ops = [add("Build the parser"), add("Ship v1.2"), add("Fix the tests")]

      assert ImportGate.shape_facts_block(ops, 3, 40) ==
               "Server-computed: this batch adds 3 tasks (40 this session)."
    end

    test "extensions need a letter — version numbers aren't path-like" do
      ops = [add("Ship v1.2"), add("notes.md"), add("weird.a1")]

      assert ImportGate.shape_facts_block(ops, 3, 3) =~
               "2 of 3 new task titles look like file paths/names."
    end

    test "non-task ops and missing fields are ignored" do
      ops = [
        %{"op" => "add", "type" => "initiative", "data" => %{"name" => "a/b.md"}},
        %{"op" => "add", "type" => "task", "data" => %{}},
        add("clean title")
      ]

      assert ImportGate.shape_facts_block(ops, 2, 2) ==
               "Server-computed: this batch adds 2 tasks (2 this session)."
    end
  end
end
