defmodule DoitMcp.Application do
  ...
  def start(_type, _args) do
    ...
    # the container log (m03.04 2.5.7).
    :ok = DoitMcp.LogRedaction.install()

    # One read at boot for the frame-log switch (m03.04 2.5.5): capture
  ...
  def children(_env) do
    [
      ...
      # (m03.04 2.5.3) — same never-race-a-tool-call ordering.
      {Registry, keys: :unique, name: DoitMcp.Elicitation.Registry},
      DoitMcp.TokenRecovery.Sessions,
      # Per-session frame capture (m03.04 2.5.5) — before the transport
      ...
      # idle expiry is right here (m03.04 2.5.4) — an expired session
      ...
      # path, everything else the stock plug (m03.04 2.5.3; see
