defmodule DoitMcp.ImportGate do
  ...
  # rather than authored how-text. Retunable.
  @long_description_chars 2_000

  @doc """
  Server-computed shape facts about a held batch, for the gate's confirm form
  (m03.04 3.1 iteration 1) — counts the agent's readback can't gloss over.
  Always states the batch size; mirror-import tells (path-like task-add
  titles, whole-file-sized descriptions) appear only when present, with one
  line naming the pattern. Pure — formatting lives here so the form and the
  tests share one source.
  """
  @spec shape_facts_block([map()], pos_integer(), pos_integer()) :: String.t()
  def shape_facts_block(operations, task_adds, cumulative) do
    adds = Enum.filter(operations, &task_add?/1)

    pathlike = Enum.count(adds, &pathlike_title?(add_field(&1, "title")))
    ...
    tells =
      Enum.filter(
        [
          pathlike > 0 &&
            "#{pathlike} of #{length(adds)} new task titles look like file paths/names.",
          long_descriptions > 0 &&
            "#{long_descriptions} new descriptions run #{@long_description_chars}+ " <>
              "characters (whole-file sized)."
        ],
        &is_binary/1
      )

    size = "Server-computed: this batch adds #{task_adds} tasks (#{cumulative} this session)."
    ...
    case tells do
      ...
      tells ->
        Enum.join(
          [size | tells] ++
            [
              "High counts are file-mirror tells — one task per source file copies " <>
                "the filesystem instead of extracting the work."
            ],
          "\n"
        )
    end
  ...
  # dot-extension (letter-led, so "Ship v1.2" doesn't count).
