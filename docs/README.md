...
- **[`COPY_IDEAS.md`](COPY_IDEAS.md)** — optional copy ideas, with no commitment to use them.
...
PLAN.md
  Milestone     coherent shippable slice of the product
    ...
    Worklist  checkable, sequenced list of Items belonging to one Arc
...
A Worklist is an actionable, checkable parent. Its checkbox records whether the Worklist's Items are complete; the Items nest beneath it.
...
1.4 Worklists and Items use plain Markdown numbered checkboxes; Subitems use nested checkboxes. In an Arc doc, each Worklist is a top-level checkbox whose Items nest beneath it. Milestone and Arc titles remain non-checkable headings or table entries. **Worklists and Items must not prefix themselves with their own dotted reference** — position in the doc gives the reference. `1. [ ] Audit the app...` ✓ — `1. [ ] **m02.01.01** — Audit the app...` ✗.
