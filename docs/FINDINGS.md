...
2. Statuses: `open` (just observed), `triaged` (assessed and routed), `promoted` (scheduled into an action list with a link to the owning milestone/worklist), `fixed` (with a link to the resolving change), `deferred` (intentionally not scheduled, with the reason).
...
6. Promotion is explicit: when a finding becomes scheduled work, change its status to `promoted` and link the owning milestone/worklist.
