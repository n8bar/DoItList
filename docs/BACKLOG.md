...
### Initiative settings
- **Progress calc mode.** The first-generation average (each direct child counts as one unit — the original M01 formula) returns as a per-initiative setting; the M02 leaf-average default stays the default. Home exists: the initiative pane's Settings section (.03.07.05).
