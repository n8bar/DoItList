...
- **Hide fully completed tasks (workspace toggle).** An obvious toggle on the tree that hides 100%-done tasks — obvious meaning its ON state stays loudly visible as a signifier, so an accidental click never leaves someone hunting for tasks that are merely filtered. Candidate seed of a later multi-filter feature (status/assignee/priority sharing the same visible-filter-state treatment).
