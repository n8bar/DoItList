...
- **Admin role.** A delegated tier between owner and editor: manage the roster (add / remove members, change roles below admin) without ownership — no initiative delete, no ownership transfer. Lets owners hand off member management; pairs with the invite system (see *Email infrastructure & invites*). When this lands, ownership transfer demotes the old owner to **admin** instead of today's editor.
...
### Monetization
- **Donation screen.** A "support this project" page so the app can be monetized — suggested amounts + a payment-processor integration (Stripe or similar), linked from the nav / account menu. Brings a payment-processor dependency.
