...
- **Edits made on a dead connection are lost silently.** A title edited while the LiveView channel is down sits in the channel's push buffer with no signifier; when the client library's recovery hard-refresh fires (a long-poll join timeout), the buffer and the edit vanish and nothing ever says so. Seen 2026-09-10: the operator's Delivery Plan tab sent nothing for eleven minutes, reloaded on its own, and two retitles disappeared. Expected: writes made while disconnected queue locally, survive a reload, replay once on rejoin, and the field reads unsaved until the server acknowledges — the outbound queue and resync semantics this milestone defines.
