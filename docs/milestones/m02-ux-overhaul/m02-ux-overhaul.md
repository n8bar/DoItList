...
### 1 — UX Guardrails Audit & Alignment

Pass over current code against the universal baseline; address gaps; record project-specific UX rules surfaced during M02.

1. [ ] Audit the running app against every rule in [`UX_GUARDRAILS.md`](../../UX_GUARDRAILS.md) § Universal Baseline. Cover desktop and mobile widths; cover both light and dark modes (after the theme toggle lands). Log each gap as a separate `F#NN` entry in [`FINDINGS.md`](../../FINDINGS.md) — observation only, no fix detail.
2. [ ] Add the project-specific UX rules surfaced by M02 to [`UX_GUARDRAILS.md`](../../UX_GUARDRAILS.md) § Project-Specific Additions:
...
3. [ ] Promote and resolve findings logged above. As findings are triaged, link them here as a sub-checklist (`F#NN — short title`); flip each to `promoted` in FINDINGS.md when scheduled and `fixed` when resolved. If any single fix is large enough to warrant a breakout, decompress to a Worklist per the conventions in [`../../README.md`](../../README.md).

### 2 — Design Refinements

Discrete UI changes scoped explicitly with the owner.

1. [ ] **Theme toggle.** Three-state control (System / Light / Dark) modeled on CryptoZing's. System is the default selection on first load (follows `prefers-color-scheme`); user choice persists per-user, server-side, so it follows them across devices. Both light and dark must look polished — neither is a graceful fallback. Place the toggle in the app chrome (header or user menu). _Land first: the audit needs both modes available to cover them._
2. [ ] **Full-width progress underbar.** Replace the current task-progress indicator with a thin colored bar pinned to the bottom edge of each task row, spanning the full row width less a small horizontal margin. Same treatment at every level of the tree. Drive from a CSS custom property (`--progress`) so updates don't shift layout.
...
- All nine Arc 2 items shipped and visible in the running app.
- Arc 1 audit complete; every finding logged as `F#NN` and either marked `fixed` (with link to the resolving change) or explicitly `deferred` (with reason).
