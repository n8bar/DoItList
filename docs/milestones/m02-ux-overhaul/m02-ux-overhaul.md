...
17. [ ] **Fixed-height layout with main-scrollable content.** Page becomes a flex column of viewport height: `<body class="h-screen flex flex-col">`, `<header class="flex-none">`, `<main class="flex-1 overflow-y-auto">`. The header is always visible because only `<main>` scrolls — **no `position: sticky` anywhere** (sticky has too many edge cases that feel glitchy). The Initiative-header progress bar and h1 row sit inside `<main>` and scroll with content, by design — only the app chrome (Initiatives nav, theme toggle, app title) is permanent.
18. [ ] **Initiatives nav left of theme toggle.** Move the Initiatives link to the left side of the header nav cluster, separated visually from the theme toggle (divider or distinct grouping). Establishes Initiatives as primary navigation and the toggle as utility chrome.
19. [ ] **Borders on add-buttons.** Add a subtle border to the New List, New Task, and Add Member buttons so they read as a unified control family. Pairs with item 22.
20. [ ] **Branch icon redrawn with leaves.** Replace the current branch SVG (a stroked curve with two short offshoots) with a custom one: a brown branch with 2-3 small green leaves attached. Stroke-based; keeps the visual family with tree and leaf icons. If the hand-drawn version doesn't land, fall back to an AI-generated SVG.
21. [ ] **Click selected task to deselect.** Clicking a task that's currently selected closes the Details panel and clears selection. Today re-selecting the same task is a no-op; this turns the row click into a toggle.
22. [ ] **Bold green styling on add-buttons.** New List, New Task, Add Member: bold green text on the bordered pill from item 19. Visually establishes them as the primary "create" family across the app.
23. [ ] **Password show/hide toggle.** Login and Register password inputs gain an eyeball-icon button at the right edge — click toggles input `type` between `password` and `text`. Tiny JS hook handles the swap; no server round-trip.
24. [ ] **Drop weight input from the new-task line.** The inline weight number input is rarely used at create time and creates row clutter. Remove it; new tasks default to weight `1`. Editing weight stays available via the Details panel.
25. [ ] **Sibling-creation split-button.** The row-level `+ New Task` pill becomes a split-button: main click adds a child of THIS task (current behavior); a small `▶` triangle on its right opens a dropdown with one item — `+ New Task at this level` — which creates a sibling (`parent_id = this task's parent_id`). Dropdown is the home for future row-scoped actions (insert before/after, etc.).
26. [ ] **Initiatives index: own-first sort + role badge.** Sort the Initiatives index so Initiatives I own appear before ones shared with me. Each card shows my role on it (`owner` / `editor` / `viewer`) as a small badge so the relationship is obvious at a glance.
...
- All twenty-six Arc 2 items shipped and visible in the running app.
