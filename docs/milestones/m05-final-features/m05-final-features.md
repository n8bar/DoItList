...
- **Depth-weighted leaf average — a third progress mode.** A third value in the per-initiative *Progress calculation* setting (and in the account-level default), formula in [`ProductSpec.md`](../../ProductSpec.md). Leaf average stays the basis; every branch level damps what sits under it by 10%, so work decomposed late and in depth stops swamping the bar it was meant to clarify. Settings copy follows the existing pattern — "Depth-weighted — deeper leaves weigh less." The branch unit badge keeps showing the plain leaf count; fractional units are never surfaced.
