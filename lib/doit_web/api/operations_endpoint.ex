defmodule DoItWeb.Api.OperationsEndpoint do
  @moduledoc """
  The HTTP edge of the atomic mutation surface — the apply + idempotency +
  render flow, shared by every transport that exposes it (m04.01 worklist 5).

  `DoItWeb.Api.OperationsController` (`POST /api/v1/operations`, bearer token)
  and `DoItWeb.Client.OperationsController` (`POST /app/api/operations`, browser
  session) are both one-line delegations to `create/3`, so the batch semantics,
  the per-op result shape, the `Idempotency-Key` handling, and the error
  envelope can never fork between them. The engine itself — the op set, the
  per-op authorization matrix, the transaction — lives in
  ...
  **commits**, its response is stored beside a hash of the decoded payload.
  ...
  @doc """
  Apply an ordered batch of write operations, all or nothing, and render it.

  `opts` is passed straight to `Operations.apply_batch/3`; the browser surface
  passes `surface: :browser`.
  """
  def create(conn, params, opts \\ [])
