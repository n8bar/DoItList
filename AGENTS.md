...
- A new endpoint, op, field, or tool isn't done until its prose entry lands in [`docs/specs/agent_integration.md`](docs/specs/agent_integration.md). The generated tables cover the mechanical half; the prose is the half that needs a person.
...
- Keep responses under 150 words (excluding generated artifacts, files, and code) — big ideas land in pieces the operator can refine one at a time.
