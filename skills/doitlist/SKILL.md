...
- **Expansion follows sibling precedent.** In an existing Initiative, match sibling subtrees' depth — nest where content has natural sub-steps; no flat list beside deeply nested siblings. Precedent stops at the Initiative's edge: other Initiatives are other projects, never structure evidence.
...
4. Grain matches — every source level mapped, none invented; judged against the source, never another Initiative's shape.
